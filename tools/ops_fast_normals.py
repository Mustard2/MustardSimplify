                ...
                if new:
                    ...
                    if isinstance(node, bpy.types.ShaderNodeNormalMap):
                        ...
                        if socket and node.space in self.NORMAL_MAP_SPACES:
                            socket.default_value = self.NORMAL_MAP_SPACES.index(
                                node.space
                            )
                        ...
                        if bpy.app.version >= (5, 1, 0):
                            ...
                            if (
                                socket
                                and node.convention in self.NORMAL_MAP_CONVENTIONS
                            ):
                                socket.default_value = (
                                    self.NORMAL_MAP_CONVENTIONS.index(node.convention)
                                )
                            ...
                            if node.space == "TANGENT":
                                ...
                                if socket and node.base in self.NORMAL_MAP_BASES:
                                    socket.default_value = self.NORMAL_MAP_BASES.index(
                                        node.base
                                    )
                        ...
                        if socket and socket.default_value < len(
                            self.NORMAL_MAP_SPACES
                        ):
                        ...
                        if bpy.app.version >= (5, 1, 0):
                            ...
                            if socket and socket.default_value < len(
                                self.NORMAL_MAP_CONVENTIONS
                            ):
                                new.convention = self.NORMAL_MAP_CONVENTIONS[
                                    socket.default_value
                                ]
                            ...
                            if new.space == "TANGENT":
                                ...
                                if socket and socket.default_value < len(
                                    self.NORMAL_MAP_BASES
                                ):
                                    new.base = self.NORMAL_MAP_BASES[
                                        socket.default_value
                                    ]
...
def default_custom_nodes():
    use_new_nodes = (2, 81) <= bpy.app.version < (3, 2, 0)
    ...
    input = group.interface.new_socket(
        "Space", in_out="INPUT", socket_type="NodeSocketInt"
    )
    spaces = MUSTARDSIMPLIFY_OT_FastNormals.NORMAL_MAP_SPACES
    ...
    if bpy.app.version >= (5, 1, 0):
        ...
        conventions = MUSTARDSIMPLIFY_OT_FastNormals.NORMAL_MAP_CONVENTIONS
        input.default_value = conventions.index("OPENGL")
        input.min_value = 0
        input.max_value = len(conventions) - 1

        input = group.interface.new_socket(
            "Base", in_out="INPUT", socket_type="NodeSocketInt"
        )
        bases = MUSTARDSIMPLIFY_OT_FastNormals.NORMAL_MAP_BASES
...
def register():
    bpy.utils.register_class(MUSTARDSIMPLIFY_OT_FastNormals)


def unregister():
    bpy.utils.unregister_class(MUSTARDSIMPLIFY_OT_FastNormals)

...
class MUSTARDSIMPLIFY_OT_SimplifyScene(bpy.types.Operator):
    ...
    enable_simplify: BoolProperty(name="Simplify", default=True)
    ...
    def execute(self, context):
        ...
        for obj in objects:
            ...
            if (
                settings.objects
                and (eo.visibility if eo is not None else True)
                and obj.type not in objects_ignore
            ):
                if self.enable_simplify:
                    ...
                    for mod in modifiers:
                        ...
                        if addon_prefs.debug:
                            print(
                                "Modifier "
                                + mod.name
                                + " disabled (previous show_viewport: "
                                + str(status)
                                + ")."
                            )
                else:
            ...
            if (
                settings.shape_keys
                and obj.type == "MESH"
                and (eo.shape_keys if eo is not None else True)
            ):
                # Check if this mesh data has already been processed (to handle shared
                #  mesh data)
                mesh_data_processed = obj.data in processed_mesh_data
                source_obj = (
                    processed_mesh_data.get(obj.data) if mesh_data_processed else None
                )
                    ...
                    if obj.data.shape_keys is not None:
                        ...
                        if not mesh_data_processed:
                            for sk in obj.data.shape_keys.key_blocks:
                                ...
                                # We didn't use 0 to accommodate for diffeomorphic
                                # models shape-keys that use values in the range
                                # [-0.00000?, 0.00000?]with no visual effect
                                # (pretty much useless)!
                                ...
                                if has_driver(obj.data.shape_keys, attr):
                                    sk.mute = (
                                        ...
                                        if (
                                            ...
                                            and settings.shape_keys_disable_with_drivers_not_null  # noqa: E501
                                        )
                                        else settings.shape_keys_disable_with_drivers
                                    )
                                elif has_keyframe(obj.data.shape_keys, attr):
                                    sk.mute = settings.shape_keys_disable_with_keyframes
                                else:
                                    sk.mute = (
                                        (value_bool or sk.mute)
                                        if settings.shape_keys_disable_not_null
                                        else True
                                    )
                                if addon_prefs.debug:
                                    if sk.mute:
                            ...
                            # Mesh data already processed, copy the original state from
                            # the source object
                            ...
                            if (
                                ...
                                if addon_prefs.debug:
                                    print(
                                        "Shape keys for object "
                                        + obj.name
                                        + " (shared mesh data) - original state copied "
                                        "from " + source_obj.name + "."
                                    )
                else:
                    if obj.data.shape_keys is not None:
                        # Only restore shape keys if this mesh data hasn't been
                        # restored yet
                        if not mesh_data_processed:
                            # Determine source object for restoration: use current obj
                            ...
                            if len(obj_status.shape_keys) > 0:
                                ...
                                if addon_prefs.debug:
                                    print(
                                        "Using shape keys status from object "
                                        + restore_source_obj.name
                                        + " for shared mesh data (from dictionary)."
                                    )
                            ...
                            if restore_source_obj is not None:
                                ...
                                # Mark this mesh data as processed and store source
                                # object for future lookups
                                processed_mesh_data[obj.data] = restore_source_obj
                            else:
                                # No valid source object found (edge case: first
                                # disable without prior enable)
                                # Mark as processed to avoid repeated attempts, using
                                # current obj as placeholder
                                processed_mesh_data[obj.data] = obj
                                if addon_prefs.debug:
                                    print(
                                        "Shape keys for object "
                                        + obj.name
                                        + " - no saved status found, skipping "
                                        "restoration."
                                    )
                        else:
                            # Mesh data already processed (restored by first object
                            # with status)
        ...
        if settings.physics:
            ...
            if context.scene.rigidbody_world:
                ...
                if self.enable_simplify:
                    ...
                    if addon_prefs.debug:
                        print(
                            "Rigid Body World disabled (previous status: "
                            + str(status)
                            + ")."
                        )
                else:
                    scene.rigidbody_world.enabled = (
                        scene.MustardSimplify_Status.rigidbody_world
                    )
                    if addon_prefs.debug:
                        print(
                            "Rigid Body World disabled reverted to status: "
                            + str(scene.rigidbody_world.enabled)
                            + ")."
                        )
        ...
        if settings.drivers:
            ...
            for col in collections:
                ...
                for ob in collection:
                    if col == "objects":
                        eo = find_exception_obj(
                            scene.MustardSimplify_Exceptions.exceptions, ob
                        )
                    if ob.animation_data is not None:
                        for driver in ob.animation_data.drivers:
                            ...
                            if dob is not None and (
                                isinstance(dob, Vector) or isinstance(dob, Color)
                            ):
                                pp = "%s[%d]" % (pp, idx)
                            if col == "objects":
                                driver.mute = self.enable_simplify and (
                                    eo.drivers if eo is not None else True
                                )
                            else:
        ...
        try:
            ...
            print(
                "\n Mustard Simplify - An error occurred when updating the "
                "execution times"
            )
